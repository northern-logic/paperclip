...
- apiBaseUrl (string): Hermes API server base URL, for example http://127.0.0.1:8642.
  ...
  With stock multiplexed profiles, use /p/<profile> for a non-default profile and set profile to the same value.
- apiKey (string): Hermes API_SERVER_KEY. Sent as Authorization: Bearer <apiKey>.

Optional fields:
- profile (string): explicit Hermes profile for skill and instruction-file management; defaults to default.
- managementBaseUrl (string): private stock Hermes dashboard/control URL.
- managementCredential (string): HERMES_DASHBOARD_SESSION_TOKEN for the control service.
- skillBridgeBaseUrl (string): private Northern Logic profile-management bridge URL.
- skillBridgeCredential (string): bearer credential for the profile-management bridge.
- skillManagementTimeoutMs (number): per-request management deadline; defaults to 10000.
...
- Lists stock Hermes profile skills through GET /api/skills and treats Hermes-owned entries as read-only.
- Reconciles complete Paperclip-owned skill directories through the Northern Logic bundle bridge.
- Verifies assigned skills before run creation and fails closed when any assignment is missing, stale, disabled, or unobservable.
- Lists, reads, creates, updates, and deletes AGENTS.md, HEARTBEAT.md, SOUL.md, and TOOLS.md in the selected remote profile through the same authenticated bridge.
- Loads remote AGENTS.md as the next run's stable instructions when no explicit adapter or payload-template instructions override it.
