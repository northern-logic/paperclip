import type { AdapterConfigSchema } from "@paperclipai/adapter-utils";
import { DEFAULT_EVENT_RECONNECT_MS, DEFAULT_TIMEOUT_SEC } from "../shared/constants.js";
import { INSECURE_REMOTE_HTTP_ESCAPE_HATCH } from "./transport-security.js";

export function getConfigSchema(): AdapterConfigSchema {
  return {
    fields: [
      {
        key: "apiBaseUrl",
        label: "API base URL",
        type: "text",
        required: true,
        hint: "Hermes API server base URL, such as http://127.0.0.1:8642 or a private HTTPS URL. The default dashboard root or chat URL, such as http://127.0.0.1:9119/chat, is accepted and maps to /api.",
      },
      {
        key: "apiKey",
        label: "API key",
        type: "text",
        required: true,
        hint: "Hermes API_SERVER_KEY, not PAPERCLIP_API_KEY. Stored as a Paperclip secret reference.",
        meta: { secret: true },
      },
      {
        key: "profile",
        label: "Hermes profile",
        type: "text",
        default: "default",
        hint: "Explicit Hermes profile whose skills Paperclip inventories and reconciles.",
      },
      {
        key: "managementBaseUrl",
        label: "Dashboard management URL",
        type: "text",
        hint: "Private URL for the stock Hermes dashboard/control service, usually the tunnel to port 9119.",
      },
      {
        key: "managementCredential",
        label: "Dashboard management credential",
        type: "text",
        hint: "Value of HERMES_DASHBOARD_SESSION_TOKEN. Stored as a Paperclip secret reference and never sent to the browser.",
        meta: { secret: true },
      },
      {
        key: "skillBridgeBaseUrl",
        label: "Skill bundle bridge URL",
        type: "text",
        hint: "Private URL for the Northern Logic v1 skill-bundle bridge on the Hermes VM.",
      },
      {
        key: "skillBridgeCredential",
        label: "Skill bundle bridge credential",
        type: "text",
        hint: "Bearer credential for the Northern Logic skill-bundle bridge. Stored as a Paperclip secret reference.",
        meta: { secret: true },
      },
      {
        key: "skillManagementTimeoutMs",
        label: "Skill management timeout ms",
        type: "number",
        default: 10000,
        hint: "Deadline for each dashboard or bundle-bridge request.",
      },
      {
        key: INSECURE_REMOTE_HTTP_ESCAPE_HATCH,
        label: "Dangerously allow remote HTTP",
        type: "toggle",
        default: false,
        hint: "Unsafe dev-only escape hatch. Remote Hermes gateways should use HTTPS; loopback HTTP remains allowed.",
      },
      {
        key: "sessionKeyStrategy",
        label: "Session key strategy",
        type: "select",
        default: "issue",
        options: [
          { value: "issue", label: "Issue scoped" },
          { value: "agent", label: "Agent scoped" },
          { value: "run", label: "Run scoped" },
          { value: "none", label: "None" },
        ],
        hint: "Controls X-Hermes-Session-Key. Issue scoped prevents cross-task memory bleed by default.",
      },
      {
        key: "timeoutSec",
        label: "Timeout seconds",
        type: "number",
        default: DEFAULT_TIMEOUT_SEC,
      },
      {
        key: "eventReconnectMs",
        label: "Event reconnect ms",
        type: "number",
        default: DEFAULT_EVENT_RECONNECT_MS,
        hint: "Delay before reconnecting the Hermes SSE events stream after a nonterminal disconnect.",
      },
      {
        key: "paperclipApiUrl",
        label: "Paperclip API URL",
        type: "text",
        hint: "Optional Paperclip API URL reachable by the remote Hermes host. This is not a credential.",
      },
      {
        key: "headers",
        label: "Extra headers",
        type: "textarea",
        hint: "Optional JSON object of extra nonsecret headers. Security-critical headers are generated by the adapter.",
      },
      {
        key: "instructions",
        label: "Instructions",
        type: "textarea",
        hint: "Optional stable Hermes instructions sent separately from the wake input.",
      },
    ],
  };
}
